...
def sw_root(tmp_path, monkeypatch):
    ...
    for mid, name in (("111", "Installed Mod"), ("222", "Fresh Mod")):
        (root / "data" / "mods" / mid / "mod.xml").write_text(
            f'<?xml version="1.0" encoding="UTF-8"?>\n'
            f'<mod name="{name}" author="t" desc="d" workshop_id="{mid}"/>\n'
        )
    tuned = (
        'freq = property.slider("Wave Interval (Mins)", 1, 60, 1, 15)\n'
        "g_savedata = {\n"
        "    interval = freq * 60 * 60,\n"
    ...
    for name, script in (
        ...
        (d / "playlist.xml").write_text(
            f'<?xml version="1.0" encoding="UTF-8"?>\n'
            f'<playlist path_id="app_data/data/missions/x" '
            f'folder_path="data/missions/x" file_store="4" name="{name}">\n'
...
def read_scene(root: Path) -> str:
    with open(
        root / "saves" / "testsave" / "scene.xml", encoding="utf-8", newline=""
    ) as f:
        return f.read()
...
def test_settings_schema_and_full_apply_cycle(sw_root):
    ...
    savedata.save_file(
        save / "script_data" / f"{sid}.xml",
        {"interval": 15 * 3600.0, "opts": {"loud": False}},
    )
    run_cli(
        "settings",
        "testsave",
...
def test_geometry_offset_settling_and_player_protection(sw_root):
    ...
    settled = GEO_SCENE.replace(
        '30="3010.0" 31="11.5" 32="4020.0"', '30="3010.0" 31="10.6" 32="4019.1"'
    )
    vids, warns = geometry.match(settled, "Tower Pack", [])
    assert vids == [50], "physics settling within tolerance must still match"
    far = GEO_SCENE.replace(
        '30="3010.0" 31="11.5" 32="4020.0"', '30="3010.0" 31="11.5" 32="4025.0"'
    )
    vids, _ = geometry.match(far, "Tower Pack", [])
    assert vids == []
    player_spot = GEO_SCENE.replace(
        'id="60" vehicle_group_id="8"',
        'id="60" vehicle_group_id="8" ' 'is_mission="true"',
    ).replace('30="8010.0" 31="11.5" 32="9020.0"', '30="7010.0" 31="11.5" 32="7020.0"')
...
ROTATED_PLAYLIST = (
    ...
    "\t\t\t\t<components>\n"
    '\t\t\t\t\t<c component_type="3" id="1">\n'
    '\t\t\t\t\t\t<spawn_transform 00="0" 02="-1" 20="1" 22="0" '
    '30="100.0" 31="5.0" 32="200.0"/>\n'
...
def test_geometry_rotated_offset_and_tileless_locations(sw_root):
    ...
    assert vids == [50], (
        "the local offset must be rotated by the spawn "
        f"transform, not added along world axes: {warns}"
    )
    assert 60 not in vids, (
        "a location with no tile has no world coordinates - "
        "its components must never claim anything"
    )
...
def test_refresh_finds_structures_after_you_edited_the_playlist(sw_root):
    ...
    moved = TOWER_PLAYLIST.replace(
        '30="10.0" 31="20.0" 32="20.875"', '30="90.0" 31="20.0" 32="20.875"'
    )
    ...
    vids, warns = geometry.match(
        GEO_SCENE,
        "Tower Pack",
        [],
        target_playlist=snapshot.recorded("testsave", "Tower Pack"),
    )
    assert vids == [50], (
        "the tower must still be found through the playlist recorded "
        f"before the edit: {warns}"
    )
...
def test_property_tracing_idioms(sw_root):
    ...
    assert p["Despawn on delivery"].saved_path == ("GLOBALS", "DESPAWN"), (
        'a dotted assignment split across lines by "or" must be traced - '
        'this is the "x = x or default" idiom, where the stored value '
        "always wins over the script default"
    )
    assert p["Minutes"].saved_path == ("timeout",)
    assert p["Minutes"].saved_scale == 3600.0
    assert p["Automatic"].saved_path == ("auto_override",), (
        "a value read back out of g_savedata into the property's own "
        "variable is stored there too"
    )
    assert p["Seconds"].saved_path is None, (
        '"g_savedata.ask == nil" is a comparison, not an assignment, and '
        "a table constructor that merely mentions the property is not "
...
def test_settings_drift_and_reset_state(sw_root):
    ...
    assert (
        properties.drift(save, scene, lk) == []
    ), "a setting just applied cannot have drifted"
    data = savedata.load_file(sd)
    data["interval"] = 5 * 60 * 60
    savedata.save_file(sd, data)
    bad = properties.drift(save, scene, lk)
    assert (
        len(bad) == 1 and bad[0]["want"] == 30 and bad[0]["got"] == 5
    ), "the save now disagrees with what SWAM applied - that is drift"
    run_cli("reconcile", "testsave", "--no-backup")
    assert (
        properties.drift(save, Scene(save / "scene.xml"), lock.load("testsave")) == []
    )
    run_cli("remove-addon", "testsave", "Tuned Pack", "--no-backup")
    scene = Scene(save / "scene.xml")
    assert properties.drift(save, scene, lock.load("testsave")) == [], (
        "an addon this save no longer uses must not be reconciled - its "
...
def test_reset_state_alone(sw_root):
    ...
    assert not sd.is_file(), (
        "--reset-state on its own must delete the addon's saved state, not "
        "be silently ignored because no --set was given"
    )
...
def test_builtin_addon_removal(sw_root):
    ...
    assert 'path="data/missions/default_ai"' not in text, (
        "the script entry of a built-in addon is stored without the "
        '"rom/" prefix - it must be removed with the playlist entry, '
        "or the game keeps running the script"
    )
...
def test_game_path_strings():
    ...
    p = (
        Path("C:/lib/steamapps/workshop/content/573090/42")
        if paths.IS_WINDOWS
        else Path("/lib/steamapps/workshop/content/573090/42")
    )
...
def test_companion_found_when_subscribed_from_workshop(sw_root, tmp_path):
    ...
    (ws / "playlist.xml").write_text(
        '<playlist path_id="x" folder_path="x" file_store="3" '
        'name="SWAM Companion">\n</playlist>\n'
    )
...
def test_upgrade_from_edited_local_copy(sw_root):
    ...
    pl.write_text(
        pl.read_text().replace('location_id_counter="1"', 'location_id_counter="2"')
    )
...
def test_local_edits_are_detected_beyond_the_playlist(sw_root):
    ...
    rec = lock.load("testsave")["addons"]["Zone Pack"]
    (sw_root / "data" / "missions" / "Zone Pack" / "vehicle_1.xml").write_text(
        "<vehicle/>"
    )
    assert addons.local_changed(rec, "Zone Pack")
...
def test_changing_several_settings_at_once_keeps_the_script_valid(sw_root):
    ...
    src = (
        'a = property.slider("Alpha", 1, 60, 1, 15)\n'
...
def test_workshop_attached_addon_is_visible_and_removable(sw_root, tmp_path):
    ...
    run_cli(
        "remove-addon", "testsave", "Straight From Workshop", "--force", "--no-backup"
    )
...
def test_save_without_mods_can_still_be_edited(sw_root):
    ...
    with open(scene_path, encoding="utf-8", newline="") as f:
        text = f.read()
    empty = re.sub(
        r"<active_mods>.*?</active_mods>", "<active_mods/>", text, flags=re.S
    )
    ...
    run_cli("add-mod", "testsave", "222", "--no-backup")
    run_cli("remove-mod", "testsave", "222", "--no-backup")
    assert "<active_mods/>" in read_scene(
        sw_root
    ), "an emptied block must collapse back"
    run_cli("remove-addon", "testsave", "Zone Pack", "--no-backup")
...
def test_commented_out_properties_are_ignored(sw_root):
    ...
    src = (
        '-- old = property.slider("Ghost", 1, 10, 1, 5)\n'
...
def test_restore_rolls_back_the_lock_file_too(sw_root):
    ...
    run_cli("restore", "testsave")
    assert (
        lock.load("testsave")["addons"] == {}
    ), "the lock must roll back with the save"
    assert verify.run(paths.save_dir("testsave")) == []
    run_cli("restore", "testsave")
    assert "Zone Pack" in lock.load("testsave")["addons"]
    assert verify.run(paths.save_dir("testsave")) == []
...
def test_lock_knows_which_saves_share_an_addon(sw_root):
    ...
    other["addons"]["Zone Pack"] = lock.addon_record(
        "Zone Pack", "data/missions/Zone Pack", False, "x"
    )
...
def test_addon_name_with_xml_escapes(sw_root):
    ...
    task = list(companion.load_data(paths.save_dir("testsave"), sid)["tasks"].values())[
        -1
    ]
...
def test_a_save_with_no_addons_at_all(sw_root):
    ...
    run_cli("remove-addon", "testsave", "Logic Pack", "--no-backup")
    assert (
        read_scene(sw_root) == bare
    ), "an addon-less save must come back exactly as it was"
...
def test_journal_beats_geometry_when_they_disagree(sw_root):
    ...
    assert (
        50 not in vids2
    ), "a vehicle the journal gives to another addon is never claimed"
...
def test_a_zone_only_addon_never_claims_vehicles(sw_root):
    ...
    vids, oids, warns = geometry.match_all(
        GEO_SCENE, "Zones Only", ["data/missions/Zones Only"]
    )
...
def test_objects_are_matched_and_despawned_too(sw_root):
    ...
    d.mkdir()
    pl = TOWER_PLAYLIST.replace('component_type="3"', 'component_type="2"').replace(
        "Tower Pack", "Prop Pack"
    )
    (d / "playlist.xml").write_text(pl)
    scene = (
        '<dynamics object_counter="9">\n'
    ...
    vids, oids, warns = geometry.match_all(
        scene, "Prop Pack", ["data/missions/Prop Pack"]
    )
...
def test_two_swam_operations_cannot_overlap(sw_root):
    ...
    assert not list(
        (backup.paths.SWAM_DATA / "locks").glob("*.busy")
