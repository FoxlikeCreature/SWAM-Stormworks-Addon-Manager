...
def make_backup(save_path: Path, operation: str, keep: Path | None = None) -> Path:
    ...
    if not used:
        ...
        taken = {
            int(n.split("-")[2])
            for n in used
            if n.count("-") == 2 and n.split("-")[2].isdigit()
        }
    ...
    if free < need * 1.1:
        raise SystemExit(
            f"not enough room for a backup of '{save_path.name}': it needs "
            f"about {need//(1024*1024)} MB, only {free//(1024*1024)} MB free "
            f"in {root}.\nFree some space, or point SWAM_DATA_DIR at another "
            f"disk, or re-run with --no-backup if you know what you are doing"
        )
    try:
        ...
        raise SystemExit(
            f"the backup of '{save_path.name}' failed ({e}) - nothing was "
            f"changed. The half-written copy has been removed"
        )
    (root / f"{dest.name}.meta.json").write_text(
        json.dumps(
            {"operation": operation, "save": str(save_path), "time": dest.name},
            ensure_ascii=False,
            indent=1,
        )
    )
    ...
    if root.is_dir():
        for d in reversed(_sorted_dirs(root)):
            if not (root / f"{d.name}.meta.json").is_file():
                continue
            out.append(
                {"path": d, "time": d.name, "operation": _operation_of(root, d.name)}
            )
    return out
...
def acquire(save_name: str) -> Path:
    ...
    try:
        ...
        if _alive(holder) and holder != os.getpid():
            raise SystemExit(
                f"another SWAM operation is working on '{save_name}' "
                f"(process {holder}). Wait for it to finish, or delete "
                f"{p} if that process is long gone"
            )
