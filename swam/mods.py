...
def resolve_mod(ident: str) -> ModRef:
    ...
    if ident.isdigit():
        ...
        raise SystemExit(
            f"mod {ident} not found in workshop or data/mods "
            f"(if this is an addon, use add-addon/remove-addon)"
        )
...
def match_installed(installed: list[str], ident: str) -> str:
    hits = [
        w
        for w in installed
        if w == ident or w.replace("\\", "/").rstrip("/").endswith("/" + ident)
    ]
    if not hits:
...
def describe_wine_path(wine_path: str) -> str:
    ...
    for cand in [paths.local_mods_dir() / mod_id] + [
        d / mod_id for d in paths.workshop_dirs()
    ]:
        if (cand / "mod.xml").is_file():
            m = re.search(
                r'<mod name="([^"]*)"', (cand / "mod.xml").read_text(errors="replace")
            )
            if m:
                from . import addons
