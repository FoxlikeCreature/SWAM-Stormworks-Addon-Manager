    ...
    if tbl_start != -1:
        ...
        while True:
            ...
            if t.group(2) == "/":
                ...
                pos = (
                    pos + t.start() + len(f"<t {t.group(1)}>") + len(sub) + len("</t>")
                )
    return out, s
...
def load_file(path: Path) -> dict:
    try:
        ...
        raise SystemExit(
            f"{path} is not readable as addon state ({e}).\nThe game probably "
            "crashed while writing it. Restore a backup of this save "
            f"(swam backups / swam restore), or delete the file if the addon "
