...
class Scene:
    def __init__(self, path: Path):
        ...
        try:
            ...
            raise SceneError(
                f"{path} is not readable as text ({e}) - the file is "
                f"corrupted. Restore a backup: swam restore <save>"
            )
        ...
        try:
            ...
            raise SceneError(
                f"{path} looks damaged ({e}).\nThe game may have crashed "
                f"while saving. Restore a backup (swam backups / swam restore) "
                "or the save's autosave copy"
            )
    ...
    def list_playlists(self) -> list[str]:
        m = re.search(
            r"<active_playlists>.*?</active_playlists>|<active_playlists/>",
            self.text,
            re.S,
        )
        if not m:
    ...
    def list_scripts(self) -> list[dict]:
        ...
        for m in re.finditer(
            r'<s (?:script_id="(\d+)" )?store="(\d+)" path="([^"]+)"/>', tail
        ):
            out.append(
                {
                    "script_id": int(m.group(1)) if m.group(1) is not None else 0,
                    "has_id": m.group(1) is not None,
                    "store": int(m.group(2)),
                    "path": m.group(3),
                }
            )
    ...
    def verify(self) -> None:
        for tag in ("active_mods", "active_playlists", "scripts", "game_data", "scene"):
            ...
            if opens != closes:
                raise SceneError(
                    f"unbalanced <{tag}> tags: " f"{opens} opened, {closes} closed"
                )
    ...
    def diff(self) -> str:
        ...
        return "".join(
            difflib.unified_diff(
                self._orig.splitlines(keepends=True),
                self.text.splitlines(keepends=True),
                fromfile="scene.xml (before)",
                tofile="scene.xml (after)",
                n=3,
            )
        )
