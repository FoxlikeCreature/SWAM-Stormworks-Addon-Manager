...
def apply_theme(root: tk.Tk) -> None:
    ...
    style.configure(
        "TButton", background=PANEL, foreground=FG, padding=(10, 5), borderwidth=0
    )
    style.map(
        "TButton",
        background=[("disabled", BG), ("pressed", ACCENT_DIM), ("active", ACCENT)],
    ...
    style.configure(
        "ComboboxPopdownFrame", background=PANEL, borderwidth=0, relief="flat"
    )
    root.bind_class(
        "TCombobox",
        "<<ComboboxSelected>>",
    ...
    root.bind_class(
        "TCombobox", "<FocusOut>", lambda e: e.widget.selection_clear(), add="+"
    )
    ...
    style.map(
        ...
        lightcolor=[("pressed", ACCENT_DIM)],
        darkcolor=[("pressed", ACCENT_DIM)],
    )
    style.layout(
        "Horizontal.TScale",
        [
    ...
    style.map(
        "Horizontal.TScale", background=[("pressed", ACCENT_DIM), ("active", "#c86bff")]
    )
    ...
    style.configure(
        "Accent.TLabel", foreground=ACCENT, font=("TkDefaultFont", 10, "bold")
    )
    style.configure(
        "Good.TLabel", foreground="#4ade80", font=("TkDefaultFont", 10, "bold")
    )
    style.configure(
        "Bad.TLabel", foreground="#f87171", font=("TkDefaultFont", 10, "bold")
    )
    style.configure(
        "Accent.TButton",
        background=ACCENT,
    ...
    style.map(
        "Accent.TButton", background=[("pressed", ACCENT_DIM), ("active", "#c86bff")]
    )
...
class Picker(tk.Toplevel):
    ...
    def __init__(self, root, kind: str, attached: set[str]):
        ...
        sort_box = ttk.Combobox(
            ...
            state="readonly",
            width=12,
            values=("Newest first", "A-Z"),
        )
        sort_box.pack(side="right")
        ttk.Label(bar, text="Sort:", style="Muted.TLabel").pack(
            side="right", padx=(0, 4)
        )
        ...
        ttk.Label(
            manual,
            text="Click tiles to select. Type to search, "
            "or enter a workshop id/path:",
            style="Muted.TLabel",
        ).pack(anchor="w")
    ...
    def _search_changed(self):
        ...
        n = len(self.selected)
        self.confirm_btn.state(
            ["!disabled"] if n or self.var.get().strip() else ["disabled"]
        )
    ...
    def _visible_items(self, source: str) -> list:
        ...
        if query:
            items = [
                i for i in items if query in i.name.lower() or query in i.ident.lower()
            ]
    ...
    def _make_tab(self, source: str, label: str):
        ...
        canvas.bind_all(
            "<MouseWheel>",
            lambda e: self._wheel(canvas, -1 if e.delta > 0 else 1),
            add=True,
        )
    ...
    def _fill_grid(self, source: str, inner: ttk.Frame):
        ...
        for i, it in enumerate(self._visible_items(source)):
            ...
            btn = tk.Button(
                ...
                text=text,
                compound="top",
                wraplength=self.TILE_W - 12,
                justify="center",
                bg=(ACCENT if picked else PANEL),
                fg=("#ffffff" if picked else (MUTED if is_attached else FG)),
                ...
                pady=6,
                cursor="hand2",
                state=("disabled" if is_attached else "normal"),
                disabledforeground=MUTED,
                command=lambda ident=it.ident: self._toggle(ident),
            )
            btn.grid(
                row=i // self.COLS, column=i % self.COLS, padx=4, pady=4, sticky="nsew"
            )
...
HELP = [
    ...
    (
        "step",
        "1. Edit data/missions/<addon>: move a building, delete a location, whatever you wanted.",
    ),
...
class App(ttk.Frame):
    ...
    def _build(self):
        ...
        self.save_box = ttk.Combobox(
            top, textvariable=self.save_var, state="readonly", width=40
        )
        ...
        self.buttons = []
        for text, cmd in (
            ("Add addon…", self.add_addon),
            ("Add mod…", self.add_mod),
            ("Remove selected", self.remove_selected),
            ("Upgrade selected", self.upgrade_selected),
            ("Settings…", self.addon_settings),
            ("Refresh from files", self.refresh_addon),
            ("Clean leftovers…", self.clean_leftovers),
            ("Remove marked…", self.remove_marked),
            ("Install companion", self.install_companion),
            ("Check integrity", self.run_verify),
    ...
    def _two_buttons(self, title: str, text: str, primary: str, secondary: str) -> bool:
        ...
        pressed = []
        ttk.Button(
            row, text=secondary, command=lambda: (pressed.append(True), dlg.destroy())
        ).pack(side="right")
        ttk.Button(row, text=primary, style="Accent.TButton", command=dlg.destroy).pack(
            side="right", padx=(0, 8)
        )
    ...
    def _needs_game_note(self):
        messagebox.showinfo(
            "SWAM",
            "Task queued for the in-game companion.\n\n"
            "Now load this save in Stormworks, wait for the "
            '"[SWAM] tasks done" chat message, then SAVE the game.',
        )
    ...
    def refresh_items(self):
        ...
        for v in scene.list_playlists():
            ...
            if aname is None:
                self.tree.insert(
                    "",
                    "end",
                    values=("addon", v, "files not found on disk"),
                    tags=("addon", v),
                )
                continue
            if aname == companion.NAME:
                self.tree.insert(
                    "",
                    "end",
                    values=("addon", aname, "keeps the journal"),
                    tags=("companion", aname),
                )
                continue
            marks = []
        ...
        self.companion_lbl.configure(
            text="companion: installed" if comp else "companion: NOT installed",
            style="Good.TLabel" if comp else "Bad.TLabel",
        )
    ...
    def _remove_batch(self, save: str, sel):
        ...
        if inherited_:
            if not messagebox.askyesno(
                "SWAM",
                f"{len(inherited_)} of those came with the world "
                f"(inherited). Their entries can be removed, but structures "
                f"spawned at world creation stay.\n\nRemove them too?",
            ):
                inherited_ = []
        if builtin_:
            geo = messagebox.askyesno(
                "SWAM",
                f"{len(builtin_)} of those are built-in (vanilla) content. The save "
                f"stops loading them; their files stay in the game's folder, so "
                f"you can add them back from the Built-in tab.\n\n"
        ...
        def op():
            ...
            self._batch(
                mods_,
                lambda i: cmd_remove_mod(
                    _Args(
                        save=save,
                        mod=i.replace("\\", "/").rsplit("/", 1)[-1],
                        dry_run=False,
                        no_backup=False,
                    )
                ),
        ...
        self.run_op(
            f"remove {len(mods_)+len(managed_)+len(inherited_)+len(builtin_)} "
            f"item(s)",
            op,
            done=(
                self._needs_game_note if (managed_ or inherited_ or builtin_) else None
            ),
        )
    ...
    def upgrade_selected(self):
        ...
        if kind != "addon":
            messagebox.showinfo(
                "SWAM",
                "Select an addon installed by SWAM to refresh it from the "
                "workshop or from its edited local files.",
            )
            return
        ...
        if rec is None:
            messagebox.showinfo(
                "SWAM",
                f"'{ident}' was not installed by SWAM - upgrades "
                f"only work for addons it manages.",
            )
            return
        try:
        ...
        if ws_update and local_changed:
            ans = messagebox.askyesnocancel(
                "SWAM",
                f"'{ident}' has manual edits in data/missions AND a newer "
                f"workshop version.\n\n"
            ...
            if has_workshop:
                ans = messagebox.askyesnocancel(
                    "SWAM",
                    f"'{ident}' has manual edits in data/missions.\n\n"
                ...
                if not messagebox.askyesno(
                    "SWAM",
                    f"Refresh '{ident}' from its edited local files?\n\n"
                    f"On next world load the old structures are removed and "
                    f"the ones from your edited files spawned.",
                ):
                    return
                use_local = True
        else:
            if not messagebox.askyesno(
                "SWAM",
                f"Upgrade '{ident}' from its workshop version?\n\n"
                f"On next world load the old structures are removed and "
                f"the new ones spawned.",
            ):
                return
    ...
    def remove_marked(self):
        ...
        try:
            ...
            marks = (
                companion.load_data(save_dir, sid).get("marks") or {}
                if sid is not None
                else {}
            )
        except (SystemExit, OSError, ValueError) as e:
        ...
        if not marks:
            messagebox.showinfo(
                "SWAM",
                "No structures are marked in this save.\n\nIn game, stand next to the structure you want gone, type "
                '"?swam mark" in chat, SAVE the game and close it - then use '
                "this button.",
            )
            return
        ans = messagebox.askyesnocancel(
            "SWAM",
            f"{len(marks)} structure(s) marked in game.\n\n"
        ...
        def op():
            ...
            cmd_remove_marked(
                _Args(save=save, all=bool(ans), dry_run=False, no_backup=False)
            )
    ...
    def show_help(self):
        ...
        txt.tag_configure(
            "h2", foreground=FG, font=("TkDefaultFont", 10, "bold"), spacing1=8
        )
        ...
        ttk.Button(row, text="Close", style="Accent.TButton", command=dlg.destroy).pack(
            side="right"
        )
    ...
    def _check_drift(self, save_name: str, scene, lk: dict):
        ...
        lines = "\n".join(
            f"  {d['addon']}: '{d['label']}' should be {d['want']}, "
            f"{d['where']} holds {d['got']}"
            for d in bad[:6]
        )
        if not messagebox.askyesno(
            "SWAM",
            f"{len(bad)} setting(s) SWAM applied are not what the save and "
            f"the addon files actually hold now:\n\n{lines}\n\n"
            f"This happens when an addon keeps a setting in the save and "
            f"writes over the file default.\n\nPut them back?",
        ):
            return
    ...
    def refresh_addon(self):
        ...
        if not save or not sel:
            messagebox.showinfo(
                "SWAM", "Select an addon whose files you want to edit by hand."
            )
            return
        ...
        if not messagebox.askyesno(
            "SWAM",
            f"Refresh '{ident}' from its files in data/missions?\n\n"
            "Everything it placed in the world is removed and spawned again "
            "from its playlist, so your hand-edits to that playlist (moving a "
            "building, deleting one) take effect in this save.\n\n"
            "Edit the files first, then press this. SWAM keeps a copy of the "
            "playlist as it was when the structures were spawned, so it still "
    ...
    def clean_leftovers(self):
        ...
        try:
            attached = {
                addons.playlist_name(v)
                for v in Scene(paths.save_dir(save) / "scene.xml").list_playlists()
            }
        except (SystemExit, OSError) as e:
            messagebox.showinfo("SWAM", str(e))
            return
        gone = sorted(
            d.name
            for d in (paths.sw_root() / "data" / "missions").iterdir()
            if (d / "playlist.xml").is_file() and d.name not in attached
        )
        if not gone:
            messagebox.showinfo(
                "SWAM",
                "Every addon whose files are in data/missions is still attached "
                "to this save - there is nothing to clean up.\n\nThis button is "
                "for structures left behind by an addon you already removed.",
            )
            return
        pick = self._pick_one(
            "Clean leftovers",
            "An addon that is no longer attached to this save, but whose "
            "structures may still stand in the world:",
            gone,
        )
        if pick is None:
            return
    ...
    def _pick_one(self, title: str, text: str, items: list[str]) -> str | None:
        ...
        dlg.transient(self)
        dlg.grab_set()
        ttk.Label(dlg, text=text, wraplength=420, justify="left").pack(
            anchor="w", padx=12, pady=(12, 6)
        )
        var = tk.StringVar(value=items[0])
        box = ttk.Combobox(
            dlg, textvariable=var, values=items, state="readonly", width=48
        )
        ...
        ttk.Button(
            ...
            text="Clean",
            style="Accent.TButton",
            command=lambda: (out.append(var.get()), dlg.destroy()),
    ...
    def addon_settings(self):
        ...
        if kind == "builtin":
            messagebox.showinfo(
                "SWAM",
                "Built-in (vanilla) addons keep their files inside the game "
                "installation, which SWAM does not modify - their settings "
                "cannot be edited.",
            )
            return
        ...
        if not props:
            messagebox.showinfo(
                "SWAM",
                f"'{aname}' exposes no settings (its script defines "
                f"no property sliders or checkboxes).",
            )
            return
    ...
    def _settings_dialog(self, save: str, aname: str, props):
        ...
        for seq, d in (("<Button-4>", -1), ("<Button-5>", 1)):
            canvas.bind_all(
                seq,
                lambda e, d=d: canvas.winfo_exists()
                and canvas.yview_scroll(d * 2, "units"),
                add=True,
            )
        canvas.bind_all(
            "<MouseWheel>",
            lambda e: canvas.winfo_exists()
            and canvas.yview_scroll(-1 if e.delta > 0 else 1, "units"),
            add=True,
        )
        ...
        for row, p in enumerate(props):
            ...
            ttk.Label(inner, text=p.label, wraplength=340, justify="left").grid(
            ...
            if p.saved_value is None:
                src = "all saves"
            elif properties._num(p.clamp(p.saved_value)) != properties._num(
                p.clamp(p.default)
            ):
        ...
        note = ttk.Label(
            ...
            wraplength=600,
            justify="left",
            text='"this save" means the addon wrote the value into this save when '
            "the world was made, so changing it here changes this world (and "
            "the addon's own default, so new worlds start the same). "
            '"all saves" means the value only lives in the addon\'s files and '
            "every save using them reads it on each world load.",
        )
        ...
        ttk.Button(row_f, text="Apply", style="Accent.TButton", command=do_apply).pack(
            side="right", padx=(0, 8)
        )
        dlg.bind("<Escape>", lambda e: dlg.destroy())
    ...
    def add_addon(self):
        ...
        def op():
            ...
            self._batch(
                idents,
                lambda i: cmd_add_addon(
                    _Args(save=save, addon=i, dry_run=False, no_backup=False)
                ),
            )

        self.run_op(
            f"add {len(idents)} addon(s)",
            op,
            done=self._needs_game_note if has_companion else None,
        )
    ...
    def add_mod(self):
        ...
        def op():
            ...
            self._batch(
                idents,
                lambda i: cmd_add_mod(
                    _Args(save=save, mod=i, dry_run=False, no_backup=False)
                ),
            )
    ...
    def remove_selected(self):
        ...
        if kind == "companion":
            wants_more = self._two_buttons(
                "SWAM",
                "You really shouldn't do that.",
                primary="OK",
                secondary="But I need...",
            )
            if not wants_more:
                return
            delete_anyway = self._two_buttons(
                "SWAM",
                "The companion is SWAM's agent inside the game. It keeps "
                "the provenance journal - the record of which addon spawned "
                "which vehicles and objects - and it is the one who spawns "
                "and removes structures when you add or remove addons.\n\n"
                "If you delete it:\n"
                "  • the journal for this save is lost forever;\n"
                "  • addons installed so far can no longer be removed "
                "cleanly (their structures stay in the world);\n"
                "  • any queued tasks are lost.\n\n"
                "The save itself keeps working. You can reinstall the "
                "companion later, but it starts with an empty journal.",
                primary="nvm, go back",
            ...
            def op():
                ...
                cmd_uninstall_companion(
                    _Args(save=save, really=True, dry_run=False, no_backup=False)
                )
        ...
        if kind == "builtin":
            if not messagebox.askyesno(
                "SWAM",
                f"Remove built-in addon '{ident}'?\n\nThe save stops loading it. "
                f"Its files stay in the game's folder, so you can add it back "
                f"from the Built-in tab at any time.",
            ):
                return
            geometry = messagebox.askyesno(
                "SWAM",
                "Also remove the structures it placed in the world?\n\n"
            ...
            if not messagebox.askyesno(
                "SWAM",
                f"'{ident}' was not installed by SWAM (it came with the "
                f"world).\n\nIts entries can be removed, but structures "
                f"it spawned at world creation stay unless matched by "
                f"coordinates.\n\nRemove it anyway?",
            ):
                return
            force = True
            geometry = messagebox.askyesno(
                "SWAM",
                "Also try to remove its static structures by coordinate "
                "matching?\n\n(Safe matches only; anything ambiguous is "
                "left alone.)",
            )
        ...
        def op():
            ...
            cmd_remove_addon(
                _Args(
                    ...
                    no_backup=False,
                    force=force,
                    force_geometry=geometry,
                )
            )

        self.run_op(
            f"remove addon {ident}",
            op,
            done=self._needs_game_note if has_companion else None,
        )
    ...
    def restore_backup(self):
        ...
        if not entries:
            messagebox.showinfo(
                "SWAM",
                "No backups for this save yet - " "they appear after the first change.",
            )
            return
        dlg = tk.Toplevel(self.root)
        ...
        dlg.grab_set()
        ttk.Label(
            dlg, text=f"Backups of '{save}', newest first:", padding=(10, 10, 10, 4)
        ).pack(anchor="w")
        lb = tk.Listbox(
            dlg,
            width=52,
            height=min(8, len(entries)),
            bg=PANEL,
            fg=FG,
            selectbackground=ACCENT,
        ...
        ttk.Button(row, text="Cancel", command=dlg.destroy).pack(side="right")
        ttk.Button(
            row, text="Restore", style="Accent.TButton", command=do_restore
        ).pack(side="right", padx=(0, 8))
        dlg.wait_window()
        if not chosen:
            return
        ...
        if not messagebox.askyesno(
            "SWAM",
            f"Roll '{save}' back to {target['time']}?\n\n"
            f"The current state is backed up first, so this is "
            f"reversible.",
        ):
            return
        ...
        def op():
            ...
            cmd_restore(
                _Args(save=save, time=target["time"], dry_run=False, no_backup=False)
            )
...
def _explain_crash(exc: BaseException) -> None:
    ...
    try:
        messagebox.showerror(
            "SWAM - something went wrong",
            "SWAM hit an unexpected problem and cannot continue this "
            "action.\n\nNothing was half-written: every change is either "
            "completed or rolled back, and backups are kept in the swam "
            "backups folder.\n\nIf you report this on GitHub, include the "
            "text below:\n\n" + detail[-1500:],
        )
    except tk.TclError:
