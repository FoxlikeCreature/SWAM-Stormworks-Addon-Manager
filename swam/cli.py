...
def cmd_add_mod(args):
    ...
    if ref.has_tiles:
        print(
            "WARNING: this mod contains tiles - it will change terrain, "
            "including under anything already built there"
        )
    scene = Scene(save / "scene.xml")
...
def cmd_add_addon(args):
    ...
    if already is not None:
        raise SystemExit(
            f"'{ref.name}' is already attached to this save (as "
            f"{already}) - adding it again would give the game two "
            f"copies of the same addon"
        )
    ...
    with Transaction(save, f"add-addon {ref.name}", enabled=not args.no_backup) as tx:
        ...
        rec = lock.addon_record(
            ref.name,
            ref.playlist_value,
            ref.scripted,
            lock.file_hash(dest / "playlist.xml"),
            source=ref.source,
            world_digest=addons.world_digest(dest),
        )
        ...
        if sid is not None:
            ...
            print(
                "companion task queued: spawn the addon's structures on "
                "next world load (save the game afterwards)"
            )
        else:
            rec["pending_spawn"] = True
            lock.store(save.name, lk)
            print(
                "companion is not installed - the addon's structures cannot "
                "spawn yet.\nInstall the companion (swam install-companion) "
                "and SWAM will spawn them then"
            )
    if tx.backup:
        print(f"backup: {tx.backup}")
    print("done")
...
def cmd_remove_addon(args):
    ...
    if name == companion.NAME:
        raise SystemExit(
            ...
            "The companion is the one holding the cleanup journal - deleting "
            "the janitor along with his notebook is the single job he cannot "
            "do. If you truly want it gone: swam uninstall-companion"
        )
    ...
    if builtin:
        managed = False
        print(
            f"'{name}' is built-in (vanilla) content - the save stops loading "
            f"it. Its files stay in the game's rom folder, so it can be added "
            f"back at any time"
        )
    if not managed and not builtin and not args.force:
        raise SystemExit(
            f"addon '{name}' was not installed by SWAM (inherited).\n"
            f"Its entries can be removed, but structures spawned at world "
            f"creation will remain (the companion only removes what it has "
    ...
    if args.dry_run:
        ...
        if removed_sid is not None:
            print(
                f"(dry-run: script_data/{removed_sid}.xml would be deleted; "
                f"the numbering gap is kept - the game stores it as is)"
            )
        return
    with Transaction(save, f"remove-addon {name}", enabled=not args.no_backup) as tx:
        ...
        if sid is not None:
            ...
            if args.force_geometry and not managed:
                ...
                others = {
                    int(x)
                    for aname, rec in jr_all.items()
                    if aname != name
                    for kind in ("v", "o")
                    for x in rec.get(kind, {}).values()
                }
                geo_vids, geo_oids, warns = geometry.match_all(
                    scene.text,
                    name,
                    scene.list_playlists() + [value],
                    owned_elsewhere=others,
                    target_playlist=snapshot.recorded(save.name, name),
                )
                for w in warns:
                    print(f"  geometry: {w}")
                ...
                for pool, found in ((vehicles, geo_vids), (objects, geo_oids)):
                    ...
                    for x in found:
                        if x not in known:
                            ...
                            pool[base + n] = float(x)
                print(
                    f"  geometry: matched {len(geo_vids)} vehicles and "
                    f"{len(geo_oids)} objects, {added} new queued for despawn"
                )
            if vehicles or objects:
                companion.queue_task(
                    save,
                    sid,
                    {
                        "action": "despawn",
                ...
                print(
                    f"companion task queued: remove {len(vehicles)} "
                    f"vehicles and {len(objects)} objects on next world "
                    f"load (save the game afterwards)"
                )
            else:
                dropped = companion.cancel_pending(save, sid, name)
                if dropped:
                    print(
                        f"cancelled {dropped} queued task(s) for this addon - the "
                        "game never ran them, so nothing was spawned"
                    )
    if tx.backup:
        print(f"backup: {tx.backup}")
...
def cmd_install_companion(args):
    ...
    if entry is not None:
        if entry["store"] == 3:
            print(
                "companion already installed in this save - it comes straight "
                "from the workshop, so Steam keeps it up to date (SWAM leaves "
                "those files alone)"
            )
            return
    ...
    with Transaction(save, "install-companion", enabled=not args.no_backup) as tx:
        ...
        if waiting:
            lock.store(save.name, lk)
            print(
                f"queued the structures of {len(waiting)} addon(s) added before "
                f"the companion existed: {', '.join(waiting)}"
            )
    if tx.backup:
        print(f"backup: {tx.backup}")
    print(
        f"companion installed (script_id={sid}). It starts keeping the "
        f"provenance journal after the save is loaded; chat command: ?swam"
    )
    if waiting:
        print(
            'load the save once, wait for the "[SWAM] tasks done" chat '
            "message, then save the game"
        )
...
def cmd_upgrade_addon(args):
    ...
    if name not in lk["addons"]:
        raise SystemExit(
            f"addon '{name}' is not managed by SWAM - upgrade "
            f"only works for addons it installed"
        )
    rec = lk["addons"][name]
    local_dir = paths.sw_root() / "data" / "missions" / name
    if not (local_dir / "playlist.xml").is_file():
        raise SystemExit(
            f"{local_dir} has no playlist.xml - the addon's local "
            f"files are gone, nothing to work from"
        )
    ...
    if args.discard_local and src is None:
        raise SystemExit(
            f"'{name}' has no workshop source to fall back to "
            f"(it was installed from local files)"
        )
    ...
    if ws_update and local_changed and not (args.local or args.discard_local):
        raise SystemExit(
            f"the local copy of '{name}' has manual edits AND the "
            f"workshop has a newer version. Pick one:\n"
    ...
    if not from_workshop and not local_changed:
        print(
            "already up to date (script.lua edits in data/missions apply on "
            "every load by themselves - only playlist and location files need "
            "an upgrade)"
        )
        return
    if from_workshop and src.name != name:
        raise SystemExit(
            f"the workshop version renamed the addon to "
            f"'{src.name}' - that changes save entries; "
            f"remove and re-add instead"
        )
    sid = companion.script_id(scene)
    if sid is None:
        raise SystemExit(
            "the companion is required for upgrades " "(swam install-companion)"
        )
    shared = lock.other_saves_using(name, save.name)
    if shared:
        print(
            f"note: {len(shared)} other save(s) use the same files in "
            f"data/missions ({', '.join(shared[:3])}"
            f"{'...'if len(shared)>3 else''}). They get the new version too, "
            f"but their structures are only refreshed when you upgrade the "
            f"addon there as well"
        )
    source_desc = str(src.disk_path) if from_workshop else f"local edits in {local_dir}"
    if args.dry_run:
        steps = (
            "despawn old structures -> replace files -> respawn"
            if from_workshop
            else "despawn old structures -> respawn from the edited local files"
        )
    ...
    print(
        f"upgraded from {what}. On next world load the companion removes "
        f"the old structures and spawns the new ones (in that order) - "
        f"then save"
    )
...
def cmd_cleanup(args):
    ...
    if sid is None:
        raise SystemExit(
            "the companion is required for cleanup " "(swam install-companion)"
        )
    ...
    vids, oids, warns = geometry.match_all(
        scene.text,
        name,
        scene.list_playlists(),
        owned_elsewhere=known_v | known_o,
        target_playlist=snapshot.recorded(save.name, name),
    )
    for w in warns:
        print(f"  geometry: {w}")
    ...
    if args.dry_run:
        print(
            f"(dry-run) would despawn {len(vids)} vehicles {sorted(vids)} "
            f"and {len(oids)} objects {sorted(oids)}"
        )
        return
    with Transaction(save, f"cleanup {name}", enabled=not args.no_backup) as tx:
        companion.queue_task(
            ...
            sid,
            {
                "action": "despawn",
                "addon": name,
                "vehicles": {i + 1: float(v) for i, v in enumerate(sorted(vids))},
                "objects": {i + 1: float(o) for i, o in enumerate(sorted(oids))},
            },
        )
    if tx.backup:
        print(f"backup: {tx.backup}")
    print(
        f"queued: {len(vids)} vehicles and {len(oids)} objects will be "
        f"despawned on next world load (save the game afterwards)"
    )
...
def cmd_refresh(args):
    ...
    if not (local_dir / "playlist.xml").is_file():
        raise SystemExit(
            f"{local_dir} has no playlist.xml - refresh works on the "
            f"local copy of an addon, there is nothing to spawn from"
        )
    sid = companion.script_id(scene)
    if sid is None:
        raise SystemExit(
            "the companion is required for refresh " "(swam install-companion)"
        )
    jr = companion.journal(save, sid)
    mine = jr.get(name, {})
    known_v = {int(x) for rec in jr.values() for x in rec.get("v", {}).values()}
    ...
    if as_spawned is None:
        print(
            f"note: SWAM has no record of how '{name}' looked when its "
            f"structures were spawned, so it matches them against the files "
            f"as they are now. If you have already edited them, the old "
            f"structures may not be found - restore the files, run this "
            f"again, then edit"
        )
    elif snapshot.edited(save.name, name, local_dir):
        print(
            "your edits are noticed - the old structures are matched "
            "against the version SWAM recorded before them"
        )
    vids, oids, warns = geometry.match_all(
        scene.text,
        name,
        ...
        - {
            int(x)
            for x in list(mine.get("v", {}).values()) + list(mine.get("o", {}).values())
        },
        target_playlist=as_spawned,
    )
    for w in warns:
        print(f"  geometry: {w}")
    ...
    if args.dry_run:
        print(
            f"(dry-run) would despawn {len(vids)} vehicles and {len(oids)} "
            f"objects, then respawn '{name}' from {local_dir}"
        )
        return
        ...
        if vids or oids:
            companion.queue_task(
                ...
                sid,
                {
                    "action": "despawn",
                    "addon": name,
                    "vehicles": {i + 1: float(v) for i, v in enumerate(vids)},
                    "objects": {i + 1: float(o) for i, o in enumerate(oids)},
                },
            )
        companion.queue_task(save, sid, {"action": "spawn_env", "addon": name})
    ...
    print(
        f"queued: {len(vids)} vehicles and {len(oids)} objects will be "
        f"despawned, then '{name}' will be spawned again from your files.\n"
...
def cmd_reconcile(args):
    ...
    if not bad:
        print(
            "every setting SWAM applied is still what the save and the files "
            "actually hold"
        )
        return
    for d in bad:
        print(
            f"drift: '{d['addon']}' / '{d['label']}' should be {d['want']}, "
            f"but {d['where']} holds {d['got']}"
        )
    if args.dry_run:
...
def cmd_remove_marked(args):
    ...
    if not marks:
        raise SystemExit(
            "no marks in this save.\nIn game, stand next to the structure "
            'you want gone and type "?swam mark" in chat, then SAVE the '
            "game, close it and re-run this command."
        )
    vehicles = geometry.scene_vehicles(scene.text)
    attested = geometry.addon_attested(scene.text)
    known = {
        int(v)
        for rec in data.get("journal", {}).values()
        for v in rec.get("v", {}).values()
    }
    ...
    def group_ok(gid):
        return all(v in attested for v in groups[gid]) and not any(
            v in known for v in groups[gid]
        )
    ...
    for n in sorted(marks, key=str):
        ...
        for v in near:
            ...
            if any(i in known for i in groups[gid]):
                print(
                    f"mark {n}: skipping group {gid} - it belongs to a "
                    f"managed addon (see swam journal); remove that addon "
                    f"instead"
                )
            else:
                print(
                    f"mark {n}: skipping group {gid} - the game does not "
                    f"mark it as addon-spawned (players built it?)"
                )
        if picked is None:
            if not near:
        ...
        print(
            f"mark {n}: vehicle group {picked['group_id']} "
            f"({len(groups[picked['group_id']])} vehicle(s), "
            f"{d(picked):.0f} m away)"
        )
        if args.all:
            ...
            twins = [
                g
                for g in groups
                if g != picked["group_id"] and group_ok(g) and group_digest(g) == dig
            ]
            for g in twins:
                target_gids.add(g)
    ...
    if tx.backup:
        print(f"backup: {tx.backup}")
    print(
        f"queued: {len(vids)} vehicles ({len(target_gids)} group(s)) will "
        f"be despawned on next world load (save the game afterwards). "
        f"Marks cleared."
    )
...
def cmd_settings(args):
    ...
    if not args.set and args.reset_state:
        ...
        if tx.backup:
            print(f"backup: {tx.backup}")
        print(
            f"deleted {gone.name} - the addon starts over from its defaults on "
            f"the next world load, and whatever it had remembered is gone"
        )
        return
    if not args.set:
        ...
        for p in props:
            rng = (
                f"  [{properties._num(p.minimum)}"
                f"..{properties._num(p.maximum)}"
                f" step {properties._num(p.step)}]"
                if p.kind == "slider"
                else ""
            )
            ...
            src = (
                ...
                else "script default (not found in this save)"
            )
            if p.kind == "checkbox":
    ...
    with Transaction(save, f"settings {name}", enabled=not args.no_backup) as tx:
        ...
        if getattr(args, "reset_state", False):
            ...
            if gone is not None:
                print(
                    f"deleted {gone.name} - the addon starts over from its "
                    f"defaults on the next world load, and whatever it had "
                    f"remembered is gone"
                )
    if tx.backup:
        print(f"backup: {tx.backup}")
...
def cmd_uninstall_companion(args):
    ...
    if not args.really:
        raise SystemExit(
            "This deletes the provenance journal for this save - SWAM will "
            "forget who spawned what,\nand clean removal of addons installed "
            "so far becomes impossible.\nIf you mean it, re-run with --really"
        )
    ...
    if tx.backup:
        print(f"backup: {tx.backup}")
    print(
        "companion uninstalled. It cleaned out its desk and left the "
        "building. The journal went with it."
    )
...
def cmd_restore(args):
    ...
    if args.time:
        ...
        if not matches:
            raise SystemExit(
                f"no backup named {args.time} - see: " f"swam backups {args.save}"
            )
        target = matches[0]
    else:
        target = entries[0]
    if args.dry_run:
        print(
            f"(dry-run) would roll '{args.save}' back to {target['time']} "
            f"({target['operation']}), backing up the current state first"
        )
        return
    ensure_game_closed()
    ...
    print(
        f"restored '{args.save}' from {target['time']} "
        f"({target['operation']})."
        + (
            ""
            if args.no_backup
            else " The previous state was backed up as " "'pre-restore'."
        )
    )
...
def build_parser():
    p = argparse.ArgumentParser(
        prog="swam", description="addon and mod manager for existing Stormworks saves"
    )
    ...
    for name, fn, arg, help_ in (
        ("add-mod", cmd_add_mod, "mod", "attach a mod"),
        ("remove-mod", cmd_remove_mod, "mod", "detach a mod"),
        ("add-addon", cmd_add_addon, "addon", "attach an addon"),
        ("remove-addon", cmd_remove_addon, "addon", "detach an addon"),
        (
            "upgrade-addon",
            cmd_upgrade_addon,
            "addon",
            "refresh a SWAM-installed addon from the workshop "
            "or from its edited local copy",
        ),
    ):
        ...
        sp.add_argument(
            "--no-backup", action="store_true", help="skip the backup (not recommended)"
        )
        if name == "remove-addon":
            sp.add_argument(
                "--force",
                action="store_true",
                help="remove entries of an inherited addon",
            )
            sp.add_argument(
                "--force-geometry",
                action="store_true",
                help="despawn an inherited addon's structures by "
                "coordinate matching (a compromise)",
            )
        if name == "upgrade-addon":
            sp.add_argument(
                "--local",
                action="store_true",
                help="refresh the save from the (edited) local copy in "
                "data/missions instead of the workshop",
            )
            sp.add_argument(
                "--discard-local",
                action="store_true",
                help="overwrite local manual edits with the workshop version",
            )
        sp.set_defaults(fn=fn)
    for name, fn, help_ in (
        ("verify", cmd_verify, "check integrity"),
        ("status", cmd_status, "save and lock state"),
        ("journal", cmd_journal, "provenance journal"),
        ("backups", cmd_backups, "list backups"),
    ):
        sp = sub.add_parser(name, help=help_)
        sp.add_argument("save")
        sp.set_defaults(fn=fn)
    sp = sub.add_parser(
        "cleanup",
        help="despawn an addon's leftover structures by "
        "coordinate matching (works even after the "
        "addon entries were already removed)",
    )
    sp.add_argument("save")
    sp.add_argument(
        "addon", help="addon name (its files must still be in " "data/missions)"
    )
    ...
    sp = sub.add_parser(
        "refresh",
        help="despawn everything an addon placed in the world, "
        "then spawn it again from its local files - so that "
        "hand-edits to those files take effect in an existing "
        "save",
    )
    sp.add_argument("save")
    ...
    sp = sub.add_parser(
        "reconcile",
        help="check every setting SWAM applied against what the "
        "save and the addon files really hold, and put back "
    ...
    sp = sub.add_parser(
        "remove-marked", help="despawn structures marked in game with " '"?swam mark"'
    )
    sp.add_argument("save")
    sp.add_argument(
        "--all",
        action="store_true",
        help="also remove every identical structure on the map",
    )
    sp.add_argument("--dry-run", action="store_true")
    sp.add_argument("--no-backup", action="store_true")
    ...
    sp = sub.add_parser(
        "settings",
        help="view or change an addon's settings (the same "
        "sliders the game shows at world creation)",
    )
    sp.add_argument("save")
    ...
    sp.add_argument(
        "--reset-state",
        action="store_true",
        help="delete the addon's saved state so it starts over from "
        "the defaults - the last resort when the addon keeps a "
        " setting somewhere SWAM cannot find",
    )
    sp.add_argument(
    ...
    sp.add_argument(
        "--dry-run",
        action="store_true",
        help="show what would be restored, change nothing",
    )
    sp.add_argument(
        "--no-backup",
        action="store_true",
        help="do not back up the current state first " "(not recommended)",
    )
    sp.set_defaults(fn=cmd_restore)
    sp = sub.add_parser(
        "install-companion", help="install the companion addon into a save"
    )
    sp.add_argument("save")
    sp.add_argument("--dry-run", action="store_true")
    sp.add_argument("--no-backup", action="store_true")
    ...
    sp = sub.add_parser(
        "uninstall-companion", help="remove the companion (deletes the journal)"
    )
    sp.add_argument("save")
    sp.add_argument(
        "--really", action="store_true", help="confirm losing the provenance journal"
    )
    sp.add_argument("--dry-run", action="store_true")
    sp.add_argument("--no-backup", action="store_true")
