...
def script_entry(scene: Scene) -> dict | None:
    for s in scene.list_scripts():
        if s["path"] == PLAYLIST_VALUE or (
            s["store"] == 3 and _names_companion(s["path"])
        ):
            return s
    return None
...
def queue_task(save_path: Path, sid: int, task: dict) -> int:
    ...
    if dropped:
        print(
            f"dropped {dropped} queued task(s) the game had not executed "
            "yet - they are superseded by this one"
        )
...
def cancel_pending(save_path: Path, sid: int, addon: str) -> int:
    data = load_data(save_path, sid)
    dropped = drop_pending(data, "spawn_env", addon) + drop_pending(
        data, "despawn", addon
    )
    if dropped:
        savedata.save_file(_data_file(save_path, sid), data)
