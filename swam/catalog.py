    ...
    for ws in paths.workshop_dirs():
        for d in sorted(ws.iterdir()):
            ...
            if name:
                seen.add(d.name)
                items.append(
                    Item(d.name, name, _find_preview(d), "workshop", "addon", folder=d)
                )
    ...
    if missions.is_dir():
        for d in sorted(missions.iterdir()):
            if (d / "playlist.xml").is_file():
                ...
                items.append(
                    Item(d.name, name, _find_preview(d), "local", "addon", folder=d)
                )
        ...
        if rom.is_dir():
            for d in sorted(rom.iterdir()):
                if (d / "playlist.xml").is_file():
                    ...
                    items.append(
                        Item(
                            str(d), name, _find_preview(d), "builtin", "addon", folder=d
                        )
                    )
            break
    return items
    ...
    for ws in paths.workshop_dirs():
        for d in sorted(ws.iterdir()):
            ...
            if name:
                seen.add(d.name)
                items.append(
                    Item(d.name, name, _find_preview(d), "workshop", "mod", folder=d)
                )
