...
def _guard_ranges(code: str) -> list[tuple]:
    ...
    for m in re.finditer(r"\bif\b([^\n]*\bis_world_create\b[^\n]*)\bthen\b", code):
        if re.search(
            r"\bnot\s+is_world_create\b|is_world_create\s*==\s*false", m.group(1)
        ):
            continue
        depth = 1
        for t in re.finditer(
            r"\b(if|for|while|function|repeat|end|until)\b", code[m.end() :]
        ):
            depth += (
                1 if t.group(1) in ("if", "for", "while", "function", "repeat") else -1
            )
            if depth <= 0:
...
class AddonRef:
    def __init__(self, disk_path: Path):
        ...
        if bad or self.name != self.name.strip() or self.name.endswith("."):
            raise SystemExit(
                f"the addon calls itself {self.name!r}, and that name cannot "
                f"become a folder under data/missions on Windows (leading or "
                f'trailing spaces, a trailing dot, or one of <>:"/\\|?*).\n'
                f"Rename it in {pl} - both the name attribute and the folder - "
                f"and add it again"
            )
    ...
    def init_risk(self) -> str | None:
        ...
        writes = [
            ...
            for m in re.finditer(
                r"g_savedata\s*(?:\.[A-Za-z_]\w*|\[[^\]]*\])*" r"\s*=(?!=)", text
            )
        ]
        ...
        if uses_guard and not has_top_init:
            return (
                "this addon initializes only under is_world_create - in an "
                "existing save that never runs, so the addon may misbehave "
                "or crash. Check its behaviour after adding"
            )
        if uses_guard:
            return (
                "this addon has world-creation-only actions - they will be "
                "skipped (the companion spawns its env locations instead)"
            )
        return None
    ...
    for s in scene.list_scripts():
        ...
        if s["store"] == 3:
            ...
            if disk and (disk / "playlist.xml").is_file():
                m = re.search(
                    r'<playlist [^>]*name="([^"]*)"',
                    (disk / "playlist.xml").read_text(errors="replace"),
                )
    ...
    if _WS_INDEX is None or refresh:
        ...
        for ws in paths.workshop_dirs():
            for d in ws.iterdir():
                ...
                if pl.is_file():
                    m = re.search(
                        r'<playlist [^>]*name="([^"]*)"', pl.read_text(errors="replace")
                    )
...
def install_files(ref: AddonRef) -> Path:
    ...
    if dest.exists():
        if dir_digest(dest) == dir_digest(ref.disk_path):
            return dest
        raise SystemExit(
            f"{dest} already holds a DIFFERENT copy of '{ref.name}' "
            f"(its files do not match the source).\nAnother save may be "
            f"using it. Sort it out manually - refusing to overwrite"
        )
