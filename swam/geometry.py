    ...
    for l in re.finditer(r"<l ([^>]*)>(.*?)</l>", whole, re.S):
        ...
        for c in re.finditer(r'<c component_type="(\d+)"[^>]*>(.*?)</c>', text, re.S):
            ...
            if "30" not in a:
                continue
            m = {
                f"{r}{c2}": float(a.get(f"{r}{c2}", 1.0 if r == c2 else 0.0))
                for r in range(4)
                for c2 in range(4)
            }
            ...
            if om:
                off.update(
                    {
                        k: float(v)
                        for k, v in re.findall(r'([xyz])="([-\d.eE]+)"', om.group(1))
                    }
                )
...
def scene_groups(scene_text: str) -> list[dict]:
    ...
    for m in re.finditer(
        r'<group id="(\d+)"[^>]*>(.*?)</group>', scene_text[i:j], re.S
    ):
        body = m.group(2)
        ...
        out.append(
            {
                ...
                "sig": _sig(
                    float(a["30"]), float(a.get("31", 0)), float(a.get("32", 0))
                ),
                "vehicles": vids,
            }
        )
...
def scene_vehicles(scene_text: str) -> list[dict]:
    out = []
    for m in re.finditer(
        r'<vehicle id="(\d+)" vehicle_group_id="(\d+)"'
        r"[^>]*>\s*<transform ([^/]*)/>",
        scene_text,
    ):
...
def scene_objects(scene_text: str) -> list[dict]:
    ...
    for m in re.finditer(r"<object ([^>]*)>(.*?)</object>", scene_text, re.S):
        ...
        out.append(
            {
                "id": int(attrs["id"]),
                "attested": attrs.get("is_mission") == "true",
                "pos": (float(a["30"]), float(a.get("31", 0)), float(a.get("32", 0))),
            }
        )
    return out
...
def match_objects(
    ...
    for o in scene_objects(scene_text):
        ...
        if o["id"] in owned_elsewhere:
            warnings.append(
                f"object {o['id']} sits on a spawn point but the "
                f"journal says another addon spawned it - leaving it alone"
            )
            continue
        if not o["attested"]:
            warnings.append(
                f"object {o['id']} sits on a spawn point but the game "
                f"does not mark it as addon-spawned - leaving it alone"
            )
            continue
        claims.setdefault(
            min(near, key=lambda t: _dist(t, _sig(*o["pos"]))), []
        ).append(o["id"])
    oids = []
...
def addon_attested(scene_text: str) -> set[int]:
    ok = set()
    for m in re.finditer(
        r'<vehicle id="(\d+)"([^>]*)>(.*?)</vehicle>', scene_text, re.S
    ):
        attrs = m.group(2)
        if ('is_mission="true"' in attrs or "addon_tags=" in attrs) and re.search(
            r"<authors\s*/>", m.group(3)
        ):
...
def _is(value: str, addon_name: str) -> bool:
    ...
    return (
        addons.playlist_name(value) == addon_name
        or value.rsplit("/", 1)[-1] == addon_name
    )
...
def match(
    ...
    tolerance: float = TOLERANCE,
    owned_elsewhere: set | None = None,
    target_playlist: Path | None = None,
) -> tuple[list[int], list[str]]:
    warnings = []
    owned_elsewhere = owned_elsewhere or set()
    if target_playlist is None:
        ...
        if target_dir is None:
            raise SystemExit(
                f"no playlist.xml found for '{addon_name}' - without its files "
                f"there is nothing to match the structures against"
            )
    ...
    if contested:
        warnings.append(
            f"{len(contested)} signatures are shared with other "
            f"addons - leaving those structures alone"
        )
        target -= contested
    ...
    for g in scene_groups(scene_text):
        ...
        if any(v in owned_elsewhere for v in g["vehicles"]):
            warnings.append(
                f"group {g['group_id']} sits on a spawn point of "
                f"'{addon_name}', but the companion's journal says another "
                f"addon spawned it - leaving it alone"
            )
            continue
        if not all(v in attested for v in g["vehicles"]):
            warnings.append(
                f"group {g['group_id']} sits on a spawn point "
                f"but the game does not mark it as addon-spawned "
                f"- leaving it alone"
            )
            continue
    ...
    for t, groups in claims.items():
        if len(groups) > 1:
            warnings.append(
                f"{len(groups)} vehicle groups sit on the same "
                f"spawn point - ambiguous, leaving them alone"
            )
            continue
