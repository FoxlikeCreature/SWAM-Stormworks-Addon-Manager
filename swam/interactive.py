...
def run(save_name: str, dispatch) -> None:
    ...
    while True:
        ...
        print(
            f"companion: {comp}; verify: "
            f"{'ok'if not problems else f'{len(problems)} problem(s)'}"
        )
        print(
            "\ncommands: d <N> remove | a <id|path|name> add addon | "
            "m <id|path> add mod | u <N> upgrade | s <N> settings |\n"
            " k <N> cleanup leftovers | x remove marked | c companion | "
            "j journal | r restore | q quit"
        )
        try:
            line = input("> ").strip()
        ...
        try:
            ...
            if cmd == "d" and arg.isdigit() and 1 <= int(arg) <= len(rows):
                ...
                cmd in ("u", "s", "k") and arg.isdigit() and 1 <= int(arg) <= len(rows)
            ):
                kind, ident = rows[int(arg) - 1]
                if kind != "addon":
                ...
                all_ = (
                    input("also remove every identical copy on the map? [y/N] ")
                    .strip()
                    .lower()
                )
                dispatch(
                    "remove-marked", [save_name] + (["--all"] if all_ == "y" else [])
                )
            elif cmd == "c":
                ...
                ts = input(
                    "timestamp to restore (empty = newest, 'n' to cancel): "
                ).strip()
                if ts.lower() != "n":
