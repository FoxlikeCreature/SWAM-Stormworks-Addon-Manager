    ...
    for root in steam_roots():
        ...
        if vdf.is_file():
            for m in re.finditer(
                r'"path"\s+"([^"]+)"', vdf.read_text(errors="replace")
            ):
...
def sw_root() -> Path:
    ...
    if root is None:
        raise SystemExit(
            "Stormworks data folder not found.\n"
            "Set the SWAM_SW_ROOT environment variable to the folder that "
            "contains 'saves' and 'data'\n"
            "(Windows: %APPDATA%\\Stormworks, Linux: inside the Proton prefix "
            "steamapps/compatdata/573090)"
        )
    return root
...
def saves_dir() -> Path:
    ...
    if not d.is_dir():
        raise SystemExit(
            f"no saves found: {d}\n"
            "Has Stormworks been installed and run at least once on this "
            "computer? If the game lives in a non-standard place, set the "
            "SWAM_SW_ROOT environment variable."
        )
    return d
