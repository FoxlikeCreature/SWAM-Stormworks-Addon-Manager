...
def run(script, overrides):
    with tempfile.NamedTemporaryFile("w", delete=False, suffix=".ov") as f:
        ...
        ov = f.name
    try:
        r = subprocess.run([LUA, str(PROBE), str(script), ov],
...
def other_value(p):
    ...
    if p.kind == "slider":
        return str(p.maximum if float(p.default) != float(p.maximum)
                   else p.minimum)
...
def main():
    ...
    for d in sorted(missions.iterdir()):
        ...
        if not script.is_file() or script.stat().st_size == 0:
            continue
        props = properties.parse_schema(
            script.read_text(errors="replace"))
        if not props:
            continue
        base = run(script, {})
        if base is None:
            print(f"  skipped {d.name}: the script does not terminate "
                  f"outside the game")
            continue
        for p in props:
            ...
            if after is None:
                continue
            real = {tuple(k.lstrip(".").split("."))
                    for k in set(base) | set(after)
                    if base.get(k) != after.get(k)}
            mine = (tuple(str(x) for x in p.saved_path)
                    if p.saved_path is not None else None)
            if not real:
                if mine is None:
                    tally["not stored"] += 1
                else:
                    tally["extra"] += 1
                    print(f"  EXTRA  {d.name} | {p.label} -> {mine} "
                          f"(nothing moves at world creation; it may still "
                          f"be a value the addon persists later)")
            elif mine is None:
                tally["missed"] += 1
                print(f"  MISSED {d.name} | {p.label} really lives at "
                      f"{sorted(real)}")
            elif mine in real:
                tally["agree"] += 1
            else:
                tally["wrong"] += 1
                print(f"  WRONG  {d.name} | {p.label}: traced {mine}, "
